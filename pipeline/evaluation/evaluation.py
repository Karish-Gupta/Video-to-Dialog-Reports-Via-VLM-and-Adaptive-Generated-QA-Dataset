...
from pipeline.evaluation.averages import calculate_averages
import json
import re
import os
import argparse
...
def _extract_caption_from_output_file_NQA(output_text: str) -> str:
    ...
    if match:
        return match.group(1).strip()
    return ""
def run_evaluation(OUTPUT_DIR="pipeline/output_results_whisper", RESULTS_FOLDER="pipeline/evaluation_results", NQA=True, QA=True, SUMMARY=True):
    """
    ...
    file, evaluate it, and save results to a separate JSON file under
    RESULTS_FOLDER.
    """
